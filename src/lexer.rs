...
impl<'a> Lexer<'a> {
    ...
    pub fn scan(&mut self) -> &Vec<Token> {       
        loop {
            ...
            match &self.src[self.idx..] {
                ...
                [b':', b':', ..] => self.add_token(TokenKind::ColonColon, self.idx, 2),
                ...
                [x, ..] if x.is_ascii_alphabetic() => {
                    ...
                    match identifier.as_str() {
                        ...
                        "let" => self.static_add_token(TokenKind::Let, begin, 3),
                        "mut" => self.static_add_token(TokenKind::Mut, begin, 3),
    ...
    fn add_token(&mut self, kind: TokenKind, begin: usize, len: usize) {
        self.output.push(Token { kind, span: TextSpan::from(begin, len) });
        self.idx += len;
    }
